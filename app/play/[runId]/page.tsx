...
export default function PlayPage() {
  ...
  // 顶部累计条：历史所有选择的净累加值（含每次选择的正负数值），仅展示正值。
  const totalGains = run.choices.reduce<StatDelta>((sum, record) => { Object.entries(record.deltas).forEach(([key, value]) => { const stat = key as keyof StatDelta; sum[stat] = (sum[stat] || 0) + (value ?? 0); }); return sum; }, {});
  // Positive attribute gains of the just-made choice, shown in the outcome section.
  const choiceGains = resolvedChoice ? Object.entries(resolvedChoice.deltas).filter(([, value]) => (value ?? 0) > 0) : [];
  const sceneArt = current.illustration ?? getPortrait(run.character.portrait).src;
  const artCaption = current.illustration
    ? "固定故事 · 内置场景插图"
    : "自定义故事 · 所选角色立绘";
  return <main className="play-page"><AppHeader compact /><div className="chapter-progress"><span>{current.chapterTitle} · 第 {sceneInChapter} 幕</span><div>{chapterNumbers.map((chapter) => <i className={chapter <= current.chapter ? "active" : ""} key={chapter} />)}</div><Link href={`/map/${run.id}`}>查看人生地图</Link></div>{Object.entries(totalGains).filter(([, value]) => (value ?? 0) > 0).length > 0 && <div className="chapter-gains">{Object.entries(totalGains).filter(([, value]) => (value ?? 0) > 0).map(([key, value]) => <span key={key}>{statMeta[key]?.icon ?? ""} {statMeta[key]?.label ?? key} <b>+{value}</b></span>)}</div>}<section className="story-stage"><div className="scene-art illustrated"><Image src={sceneArt} alt={`${current.title}手绘剧情场景`} fill priority sizes="(max-width: 760px) 100vw, 52vw" /><div className="scene-vignette" /><small>{artCaption}</small></div><article className="story-panel">{run.generation?.stage === "fallback" && <p className="chapter-fallback-note">本次 AI 内容未通过检查，已切换到可继续游玩的安全版本。{run.generation.fallbackReason ? `切换原因：${run.generation.fallbackReason}` : ""}</p>}{previous && run.currentIndex > 0 && <p className="memory-echo">人物记得：{previous.memory}</p>}<p className="scene-count">{chapterLabel} · SCENE {sceneInChapter}</p><h1>{current.title}</h1><ScrollRevealText className="scene-text rich-scene" text={current.scene} />{current.dialogue && <blockquote>{current.dialogue}</blockquote>}{!resolvedChoice && (current.choices?.length ?? 0) > 0 && <div className="choices"><p>故事走到这里，{run.character.name}准备如何回应？</p>{(current.choices ?? []).map((item, index) => <button onClick={() => choose(index)} key={item.id}><b>{String.fromCharCode(65 + index)}</b><span><strong>{item.label}</strong><small>{item.hint}</small></span><em>→</em></button>)}</div>}
    {resolvedChoice && <section className="inline-outcome" id="choice-outcome"><p className="eyebrow">YOUR CHOICE · {resolvedChoice.label}</p><h2>选择之后，生活继续发生</h2><ScrollRevealText className="outcome-story" text={resolvedChoice.outcome} variant="outcome" />{choiceGains.length > 0 && <div className="gains-row">{choiceGains.map(([key, value]) => <span key={key}>{statMeta[key]?.icon ?? ""} {statMeta[key]?.label ?? key} <b>+{value}</b></span>)}</div>}<div className="consequence-grid"><div><small>获得</small><p>{resolvedChoice.gain}</p></div><div><small>代价</small><p>{resolvedChoice.cost}</p></div><div><small>仍然未知</small><p>{resolvedChoice.unknown}</p></div></div></section>}
    {current.chapterEnd && <section className="inline-coach"><p className="eyebrow">{chapterLabel} · LIFE COACH</p><h3>这一章，先在这里停一下</h3><div className="coach"><small>章末镜面 · 不替你决定</small><p>{current.coach}</p></div><small className="no-rank">Coach 从本章经历中提出问题，不提供标准答案。</small></section>}
